...
The technique allows use to bypass Windows driver signing enforcement (KMCS).
...
Injection works from Windows Vista up to 10, running on x64.
...
* The ReflectiveLoader will then use MmGetSystemRoutineAddress (assumed to be passed in as arg0) to calculate the addresses of siz functions required by the loader, namely ExAllocatePoolWithTag, ExFreePoolWithTag, IoCreateDriver, RtlImageDirectoryEntryToData, RtlImageNtHeader, and RtlQueryModuleInformation.
...
* The ReflectiveLoader will then process the newly loaded copy of its image's import table, resolving any module dependencies (assuming they are already loaded in the kernel) and their respective imported function addresses.
